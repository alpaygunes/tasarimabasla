  ...
  <div class="row">
    ...
    <div id="content" class="product-page-content" itemscope itemtype="http://schema.org/Product">
      ...
      <div class="row product-info <?php echo $this->journal2->settings->get('split_ratio'); ?>">
        ...
        <?php } ?>
        <div class="left">
          <div class="<?php echo $class; ?>" id="tasarim-kapsayici"> <!--------tasarim-kapsayici nida ben ekledim------->
            <!-- ---------------------------------- alpaygunes -------------->
            <!-- 1 id="tasarim-kapsayici" eklendi  -->
            <?php include "kedin_tasarla_onyuz_left.tpl";?>

            <!---------------------------------------------------------------- -->
            <?php if ($thumb) { ?>
            <div class="image" id = "urun-resim-alani"><!-------- urun resimleri alani nida ben ekledim------->
      ...
      <div class="right">
        <!-- ---------------------------------- alpaygunes -------------->
        <!-- 1 id="tasarim-kapsayici" eklendi  -->
        <?php include "kedin_tasarla_onyuz_right.tpl";?>
        ...
        <div id="product" class="product-options">
          ...
          <!--- ---00001  --->
          <?php if ($price) { ?>
          <div id="price-cerceve" style="width:99%!important" alpaygunes ekledi >
            <table>
              <tr>
                <td>
                  <ul class="list-unstyled price" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                    ...
                    <?php } ?>
                    <?php if ($tax) { ?>
                    <li class="price-tax" style="display: none;" alpaygunes style yi ekledi><?php echo $text_tax; ?> <?php echo $tax; ?></li>
                    <?php } ?>
                    <?php if ($points) { ?>
                ...
                <td>
                  <ul class="list-unstyled description">
                    ...
                    <?php } ?>
                    <?php endif; ?>
                    <li class="p-model" style="font-size: 12px"><?php echo $text_model; ?> <span class="p-model" itemprop="model"><?php echo $model; ?></span></li>

                    <?php if ($reward) { ?>
                    <li class="p-rewards"><?php echo $text_reward; ?> <span class="p-rewards"><?php echo $reward; ?></span></li>
                    <?php } ?>
                    <li class="p-stock" style="display: none" style yi alpaygunes ekledi><?php echo $text_stock; ?> <span class="journal-stock <?php echo isset($stock_status) ? $stock_status : ''; ?>"><?php echo $stock; ?></span></li>
          ...
          <div id="option-cerceve" style="width:99%!important" >
          ...
          <!--- ---00001  --->
            ...
            <table style="margin-bottom: 35px">
                  ...
                  <div  class="options <?php echo $this->journal2->settings->get('product_page_options_push_classes'); ?>">
                    ...
                    <div  class="option form-group<?php echo ($option['required'] ? ' required' : ''); ?> option-<?php echo $option['type']; ?>">
                      ...
                      <select name="option[<?php echo $option['product_option_id']; ?>]" id="input-option<?php echo $option['product_option_id']; ?>" class="form-control">
                        ...
                        <option value="<?php echo $option_value['product_option_value_id']; ?>" secenek_resmi ="<?php echo $option_value['image']; ?>"><?php echo $option_value['name']; ?>
              ...
              <br>
                ...
                <td>
                  <!--  seçenek resimleri buraya gelecek   -->
                </td>
              </tr>
            </table>
...
<script type="text/javascript"><!--
  $('#button-cart').on('click', function() {
    //----------------------------------------  alpaygunes--------------------
    // boş alan kontrolu BAŞ
    var bos_alan_var = [];
    $('.etiket-alani').each(function (index) {
      if($(this).val()=='' || $(this).val()==' '){
        //istemiyorum kutusu isaretlimi
        var chck_kutu = '#chckbox'+$(this).attr('hedef');
        var istemiyorum = $(chck_kutu).is(':checked')
        if(!istemiyorum){
          bos_alan_var.push("Yazı alanı " + (index+1) + " 'i boş bıraktınız.")
        }
      }
    })

    //resim alanlarının kontrolu
    for (var k in iceriklerArr) {
      if (iceriklerArr.hasOwnProperty(k)) {
        icerik              = iceriklerArr[k];
        if(icerik.tur=="resim_yer_tutucu"){
          if(typeof icerik.src=='undefined' && typeof icerik.istemiyorum_chck=='undefined'){
            $('.resim-btn').each(function (index, value) {
              if($(this).attr('hedef')==icerik.id){
                bos_alan_var.push($(this).html()+ " alanı boş bıraktınız.");
              }
            })
          }
        }
    ...
    if(bos_alan_var.length){
      var mesaj='' ;
      $.each( bos_alan_var, function( index, value ){
        mesaj += '<br>' + value;
      })
      $('#bos-uyari-metin').html(mesaj)
      ...
      $('#dialog1-tamam').click(function () {
        $( "#dialog1" ).dialog("close");
      })

    }else {
      // boş alan kontrolu SON
...
<script type="text/javascript"><!--
  ...
  //alpaygunes
  //right in içindeki centeri product-tabs in sonuna ekle
  $(document).ready(function() {
      $('.product-info center').appendTo(".product-tabs");
  });
  //--></script>
<?php echo $footer; ?>
